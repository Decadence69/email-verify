...
let Promise = require("bluebird"),
  ...
  getAddressFromTextFile =
    require("./methods/readfromfile.js").getAddressFromTextFile,
...
for (var i = 0; i < argv.length; i++) {
  if (argv[i] === "-d") {
    ...
    if (!argv[i + 1]) {
      ...
      getAddressFromTextFile(argv[i + 1]).forEach(function (val, index, array) {
        addresses.push(val);
      });
    ...
    function (val) {
      ...
      return verify(individualOptions)
        .then((info) => {
          console.log(info);
          if (info.success) {
            fs.appendFileSync("successful_emails_uk.txt", info.addr + "\n");
          }
        })
        .catch((err) => {
          console.log(err);
